    ...
    <section className="job-news-section">
      ...
      {jobs.length > 0 ? jobs.map((job) => (
        <article key={job._id} className="job-posting">
          ...
          <a href={job.link || "mailto:zamsof.forum@gmail.com?subject=Application"} className="apply-btn">Apply Now</a>
          {job.images && job.images.length > 0 && (
            <div className="job-images-container">
              {job.images.map((img, i) => (
                <img key={i} src={`${API_BASE.replace("/api","")}${img}`} alt={job.title} onError={(e) => e.target.style.display="none"} />
              ))}
            </div>
          )}
        </article>
          ...
          {news.map((n) => (
            <div key={n._id} className="news-card">
              <div className="news-content">
                ...
                {n.link && <p><a href={n.link} target="_blank" rel="noopener noreferrer">Read more</a></p>}
              </div>
              {n.images && n.images.length > 0 && (
                <div className="news-images-card">
                  {n.images.map((img, i) => (
                    <img key={i} src={`${API_BASE.replace("/api","")}${img}`} alt={n.title} onError={(e) => e.target.style.display="none"} />
                  ))}
                </div>
              )}
            </div>
